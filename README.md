...
_Generated 2026-06-15T11:42:22Z._
...
- **Media not yet archived to GitHub Releases:** 8,279 of 24,269 media items are still `pending` (15,983 archived, 66%). Until a media item is uploaded its row keeps only the original (expiring) X CDN URL.
...
- **Accounts with no recent posts (>30d):** `@Readygov` (534d), `@EROBuffalo` (474d), `@ERODetroit` (446d), `@ERO__Phoenix` (425d), `@EROAtlanta` (412d), and 22 more. These may be quiet accounts or stalled captures.
