...
Options:
  ...
  --source-profile <dir>         Logged-in profile to clone. Default: ${DEFAULTS.sourceProfile}
  --profile-root <dir>           Destination root for per-window profile clones.
...
function profileName(task) {
  const hash = createHash('sha1').update(taskId(task)).digest('hex').slice(0, 8);
...
async function runTask(task, index, total, options) {
  const id = taskId(task);
  const profileDir = path.join(options.profileRoot, profileName(task));
  const outLog = path.join(options.logDir, `skim-batch-${id}.out.log`);
  const errLog = path.join(options.logDir, `skim-batch-${id}.err.log`);
  await cloneProfile(options.sourceProfile, profileDir);
...
async function runQueue(tasks, options) {
  ...
  async function worker() {
    while (!stop && nextIndex < tasks.length) {
      ...
      nextIndex += 1;
      const result = await runTask(tasks[index], index, tasks.length, options);
      results.push(result);
      if (result.status === 2) {
        zeroFailures += 1;
      }
...
async function main() {
  ...
  console.log(`Prepared ${tasks.length} one-window skim tasks; concurrency=${options.concurrency}.`);
  ...
  console.log(`Batch complete. stoppedEarly=${report.stoppedEarly}; completed=${report.results.length}/${tasks.length}`);
  console.log(`Report: ${reportPath}`);
  process.exitCode = report.stoppedEarly || report.results.some((result) => result.status !== 0) ? 1 : 0;
}
